...
$count_sql = "SELECT COUNT(*) as total FROM (
    ...
    $order_sql = " ORDER BY MIN(id) DESC";
    if($sort == 'price_asc'){
        $order_sql = " ORDER BY MIN(price) ASC";
    }elseif($sort == 'price_desc'){
        $order_sql = " ORDER BY MIN(price) DESC";
    }elseif($sort == 'az'){
        $order_sql = " ORDER BY brand ASC";
    }elseif($sort == 'za'){
        $order_sql = " ORDER BY brand DESC";
    }
...
SELECT
    ...
    TRIM(model) AS model,
    TRIM(scale) AS scale,
    GROUP_CONCAT(CONCAT(UCASE(LEFT(TRIM(color),1)),LCASE(SUBSTRING(TRIM(color),2))) ORDER BY id SEPARATOR '||') AS colors,
    GROUP_CONCAT(price ORDER BY id SEPARATOR '||') AS prices,
    GROUP_CONCAT(stock ORDER BY id SEPARATOR '||') AS stocks,
    GROUP_CONCAT(image ORDER BY id SEPARATOR '||') AS images,
    MIN(description) AS description
FROM products
{$where_sql}
GROUP BY TRIM(LOWER(brand)), 
    TRIM(LOWER(model)), 
    TRIM(LOWER(scale))
{$order_sql}
...
<body class="bg-light">
    ...
    <div class="d-flex">
        ...
        <!-- SIDEBAR -->
        <div class="bg-white p-3 shadow-sm" style="width:250px;height:450px; border-radius: 15px;">
            <form method="GET">
                ...
                <div class="mb-3">
                    ...
                    <div class="form-check">
                        <input type="radio" name="sort" value="az" onchange="this.form.submit()"
                            <?= ($sort == 'az') ? 'checked' : '' ?>>
                        A → Z
                    </div>

                    <div class="form-check">
                        <input type="radio" name="sort" value="za" onchange="this.form.submit()"
                            <?= ($sort == 'za') ? 'checked' : '' ?>>
                        Z → A
                    </div>
                </div>
                ...
                ?>
                            ...
                            <div class="modal-content">
                                ...
                                ?>
                                        <input type="hidden" name="product_id"
                                            value="<?= htmlspecialchars($only_pid) ?>">
                    ...
                    ?>
                        ...
                        ?>
                                        ...
                                        <div class="d-flex align-items-center mb-2">
                                            <input class="form-check-input" type="radio" name="product_id"
                                                value="<?= htmlspecialchars($pid) ?>" <?= $i === 0 ? 'checked' : '' ?>
                                                required>
                                ...
                                <div class="modal-body">
                                    ...
                                    <div class="card mb-3 shadow-sm">
                                        <div
                                            ...
                                            <strong
                                                class="text-primary"><?= htmlspecialchars($rev['username']) ?></strong>
                                        ...
                                        <div class="card-body">
                                            <p class="card-text mb-2"><?= nl2br(htmlspecialchars($rev['comment'])) ?>
                                            </p>
                    ...
                    $base_url = "?search=" .urldecode($search_query) . $brand_query;
                    ?>
                            ...
                            <li class="page-item <?= ($i == $page) ? 'active' : '' ?>">
                                <a class="page-link"
                                    href="?search=<?= urlencode($search_query) ?><?= $brand_query ?>&page=<?= $i ?>">
                                    <?= $i ?>
                                </a>
                            </li>
                            <?php endfor; ?>
