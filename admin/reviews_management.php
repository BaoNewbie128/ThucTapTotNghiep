...
if (isset($_POST['edit_review_id'])) {
    ...
    if (empty($comment)) {
        $_SESSION['message'] = "Nội dung đánh giá không được để trống.";
        $_SESSION['message_type'] = "danger";
        header("Location: ?view=reviews&page=$page");
        exit;
    }
    ...
    try {
        ...
        if ($stmt->execute()) {
            $_SESSION['message'] = "Đánh giá đã được cập nhật thành công!";
            $_SESSION['message_type'] = "success";
        } else {
            $_SESSION['message'] = "Lỗi cập nhật đánh giá.";
            $_SESSION['message_type'] = "danger";
        }
        $stmt->close();
    } catch (Throwable $e) {
...
if (isset($_POST['delete_review'])) {
    ...
    try {
        ...
        if ($stmt->execute()) {
            ...
            $_SESSION['message'] = "Không thể xóa đánh giá này.";
            $_SESSION['message_type'] = "danger";
        }
        $stmt->close();
    } catch (Throwable $e) {
        $_SESSION['message'] = "Lỗi xóa đánh giá: " . $e->getMessage();
        $_SESSION['message_type'] = "danger";
    }
    ...
    <div class="row g-3 align-items-start">
        <div class="col-12 col-md-3">
            <img src="../images/<?= $r['image'] ?>" class="img-fluid rounded w-100 mb-2 mb-md-0"
                style="height:160px; object-fit:cover;" alt="<?= htmlspecialchars($r['brand'] . ' ' . $r['model']) ?>">
        </div>
...
<nav class="mt-4">
    <ul class="pagination justify-content-center">
        <li class="page-item <?= ($page <= 1) ? 'disabled' : '' ?>">
            <a class="page-link" href="?view=reviews&page=<?= $page - 1 ?>">&laquo;</a>
        </li>
        <?php for ($i = 1; $i <= $total_pages; $i++): ?>
        <li class="page-item <?= ($i == $page) ? 'active' : '' ?>">
            <a class="page-link" href="?view=reviews&page=<?= $i ?>"><?= $i ?></a>
        </li>
        <?php endfor; ?>
        <li class="page-item <?= ($page >= $total_pages) ? 'disabled' : '' ?>">
            <a class="page-link" href="?view=reviews&page=<?= $page + 1 ?>">&raquo;</a>
        </li>
    </ul>
</nav>
<?php endif; ?>
        ...
        <div class="modal-content">
            <form method="POST">
                ...
                <div class="modal-body">
                    ...
                    <textarea name="edit_comment" id="edit_comment" class="form-control" rows="4" required></textarea>
                </div>
